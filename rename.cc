...
void pipeline_t::rename2() {
   ...
   bundle_dst = 0;
   bundle_branch = 0;
   //======ASHWIN:Initialising counters======
   unsigned int chk_pt_inst=0;
   unsigned int dest_inst=0;
   for (i = 0; i < dispatch_width; i++) {
     ...
     /* if(PAY.buf[index].checkpoint)
	  ...
	  if(IS_AMO(PAY.buf[index].flags)||IS_CSR(PAY.buf[index].flags))
	  {
	   ...
	   else if(((PAY.buf[index].good_instruction)&&(actual->a_next_pc!=PAY.buf[index].next_pc))||((PAY.buf[index].good_instruction)&&(actual->a_exception))||(instr_renamed_since_last_checkpoint+i+1==max_instr_bw_checkpoints))
	  {
   ...
   // FIX_ME #2 BEGIN
   //ASHWIN:: Stalling Renamer
   if(REN->stall_reg(dest_inst))
   {
	   return;
   }
	if (REN->stall_checkpoint(chk_pt_inst))
	{
		return;
	}
   // FIX_ME #2 END
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      // FIX_ME #3 BEGIN
	  //========MOD_CPR=======================
	  //======Inserting checkpoints before renaming for Serialising instructions and exceptions
	  if(IS_AMO(PAY.buf[index].flags)||IS_CSR(PAY.buf[index].flags))
	  {
		  REN->checkpoint();
